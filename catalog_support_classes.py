...
class MQTTCatalogSubscriber(object):

    def __init__(self):
        self.catalogID = 'Brew Catalog'
        self.file = open('catalog.json', 'r')
        self.catalog_json = self.file.read()
        self.file.close()
        self.catalog_dict = json.loads(self.catalog_json)
        self.broker_dict = self.catalog_dict['broker']
        self.catalog_sub = MyMQTT(self.catalogID, self.broker_dict['ip'], self.broker_dict['port'], self)
        self.newdev_flag=0
        self.newuser_flag=0
        return
    ...
    def end(self):
        self.catalog_sub.stop()
        pass

    def subscribe(self,topic):
        self.catalog_sub.mySubscribe(topic)
        pass
