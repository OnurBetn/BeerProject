...
class AlertPublisher(object):
    ...
    def __init__(self,userID, deviceID, broker, port):
        ...
        self.broker = broker
        self.port = port
        self.clientID = self.userID+'/alert_publisher'
...
class StorageAnalytics(object):
    ...
    def threshold(self,event, tsh):
        ...
        if self.resource not in self.measures_dict:
            self.measures_dict[self.resource] = {'unit': self.event['u'], 'events': [], 'alerts_counter': 0, 'message_flag': 0}
            pass
        self.measures_dict[self.resource]['events'].append(self.new_measure)
